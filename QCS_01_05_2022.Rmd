...
For one, a much higher number of gene counts or molecule counts may indicate that there are doublets,
especially,in droplet based scRNA-seq data like 10X.
This however is smart-seq, which is not droplet based. 
However, the caveat with smartseq is that it tends to capture a higher number of mitochondrial genes (i.e. dead cells).
...
seurat_analysed <- CreateSeuratObject(seurat_analysed@assays$RNA@counts, 
                                      ...
                                      min.cells = params$qcs_ncells, min.features = 1)
```
