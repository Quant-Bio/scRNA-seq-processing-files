...
Meta_melted<-reshape2::melt(seurat_analysed@meta.data[,c("nCount_RNA",
                                                "nFeature_RNA",
                                                "percent.mt",
                                                "patient_id", ### will need to be a parameter
                                                "SingleR")],
...
count_medians<-ddply(Meta_melted[which(Meta_melted$variable=="nCount_RNA"),],
                     .(patient_id),summarise, med=median(value))
...
d$`Less than count 25quant?`<-d$Count_medians<count_quants[2]  # Should be parameterized
d$`Less than feature 25quant?`<-d$Feature_medians<feature_quants[2] # Should be parameterized
d$`Greater than mt.ercent 75quant?`<-d$Percent_mito>mt_quants[4] # Should be parameterized
...
### Remove from seurat
Idents(seurat_analysed)<-seurat_analysed@meta.data$patient_id  ### Will need to be parameterized
