...
bassez1 <- readRDS(file=params$qcs_dir_seurat)

### Change ident to original idents
Idents(bassez1)<-bassez1@meta.data[,params$qcs_indent_var]
```
...
There are several things to consider. For one, a much higher number
...
droplet based. However, the caveat with smartseq is that it tends to capture a higher number of mitochondrial genes (i.e. dead cells).
...
bassez1 <- subset(x = bassez1, 
                  subset = nFeature_RNA > max(params$qcs_std_nFeature_RNA, median(bassez1@meta.data$nFeature_RNA, na.rm = TRUE) - 3*mad(bassez1@meta.data$nFeature_RNA, constant = 1.4826, na.rm = TRUE))
...
#                     features = all.genes, 
                     verbose = FALSE,
                     vars.to.regress = unlist(strsplit(params$qcs_vars_regress, split = ",", fixed = T)))
```
...
## 1.4 - Write to file
All the QC is done, the file is save on `r params$qcs_save_file`.

```{r, eval=FALSE, include=FALSE}
write_rds(bassez1, file=params$qcs_save_file)
```
