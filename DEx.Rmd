...
if(!exists("seurats")){
  seurats = readRDS(paste0(gsub(".rds","",params$dir_seurat),params$anno_save_name,".rds"))
}
...
### First, downsample seurat, otherwise, this will take an eternity
smalls<-list()

### Calculate PC statistics (again)
for(i in 1:length(seurats)){
    Idents(seurats[[i]])<-seurats[[i]]@meta.data[,params$qcs_indent_var]
    smalls[[i]]<-subset(seurats[[i]], downsample=1000)  
    mats[[i]]<-as.data.frame(LayerData(smalls[[i]], assay="RNA", layer="counts"))
    mats[[i]]<-mats[[i]][VariableFeatures(seurats[[i]]),]
...
for(i in 1:length(pca.list)) {
    ### Make sure cell_barcode is present
    seurats[[i]]@meta.data$cell_barcode<-Cells(seurats[[i]])
...
```{r degs-table, results="asis", fig.cap="Heatmap of top 5 Differentially Expressed Genes", tidy='styler', tidy.opts=list(strict=FALSE),eval=params$dex_run_dex}
### Print the heatmaps
for (i in 1:length(seurats)){
print(glue::glue("#### {as.vector(names(seurats[i]))}\n"))
...
```{r, degs-heatmap, results="asis", fig.height=14, fig.width=16, fig.cap="Heatmap of top 5 Differentially Expressed Genes", tidy='styler', tidy.opts=list(strict=FALSE),eval=params$dex_run_dex}
...
The annotation is done, the file is save on **_`r paste0(gsub(".rds","",params$dir_seurat),params$dex_seurat_save_name,".rds")`_**.

```{r, include=FALSE, eval=params$dex_write_dex_seurat}
### All the QC is done, so now we may write to file
write_rds(seurats, file=paste0(gsub(".rds","",params$dir_seurat),params$dex_seurat_save_name,".rds"))
```
