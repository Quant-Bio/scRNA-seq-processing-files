...
Here we remove genes that only show up in a certain number of cells. You can hit the "code" button to reveal the underlying code to remove low count genes. There may be genes with zero counts across all cells too. These reduce the average expression for a cell and should be removed, as well. For our data we choose to keep only genes which are expressed in **`r params$qcs_ncells` or more cells**. 
...
Here we remove samples that are flagged as outliers. This will remove whole samples if the sample meets the following
criteria:
