...
Reference data are load with Ensembl annotations.
`ensembl=F` indicates to not convert row names to ensembl IDs.

> **_NOTE:_** This will take an eternity or crash R. 
Don't repeat if you already have the labels.
...
```{r anno_qc1, fig.height=8, fig.width = 10, warning=FALSE, fig.cap="All cell types identified and their scores"}
plotScoreHeatmap(cells)
```

```{r anno_qc2, fig.height=8, fig.width = 10, warning=FALSE, fig.cap = "Number of low-quality labels per cell assignment"}
...
```{r anno_qc1, fig.height=8, fig.width = 10, warning=FALSE, fig.cap="All cell types identified and their scores", message=FALSE}
...
## 2.3 - Pruned ambiguous cells

```{r anno_pruned, eval=FALSE}
