...
if (any(is.null(seurat_analysed), class(seurat_analysed) != "Seurat")){
...
> **_NOTE:_** Check this assumption for each analysis specifically.

```{r, fig.cap="Identify most influential PCs", tidy='styler', tidy.opts=list(strict=FALSE)}
ElbowPlot(seurat_analysed)

PCs<-which(seurat_analysed@reductions$pca@stdev>=params$dex_stdev)
```
...
Mechanics](https://iopscience.iop.org/article/10.1088/1742-5468/2008/10/P10008), 
to iteratively group cells together, with the goal of optimizing the 
...
### Identify neighbors
seurat_analysed <- FindNeighbors(seurat_analysed, dims = 1:length(PCs))

### Identify clusters
seurat_analysed <- FindClusters(seurat_analysed)

### Run uMAP and tSNE
seurat_analysed <- RunUMAP(seurat_analysed, dims = 1:length(PCs))
seurat_analysed <- RunTSNE(seurat_analysed, dims = 1:length(PCs))
...
sigs <- DE_seurat_analysed[which(DE_seurat_analysed$p_val_adj< 0.1,#params$dex_p_val_adj &
                           DE_seurat_analysed$pct.1>0.25,#params$dex_min_pct &
                           DE_seurat_analysed$avg_log2FC>0.0),]#params$dex_logfc_threshold),]
