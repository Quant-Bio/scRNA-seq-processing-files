...
params:
  ...
  var_regress: none # <none | ["nCount_RNA", "percent.mt", "cc"] # none or bracketed list, cc=cell-cycle, only use cc with standard transformation at this point in time
  num_var_feature: 3000 # 2k, 5k, 10k are recommended choices, start with 2k or 5k, 2k best for regression
  cell_cycle: true # whether or not score cell for cell cycle phase
  exp_metadata: false # enter file name if there is metadata to add, not yet supported
  cluster_algorithm: ["Louvain2"] # <Leiden | Louvain2 | Louvain | SLM > as bracketed list; leidenalg must be installed for Leiden
  cluster_resolution: [0.15, 0.2, 0.5, 0.8]
  ...
  downsample: 20000 # Number of cells to subset to for fast clustering and annotation
---
...
# TO-DO 
# 2. make cluster marker heatmap into complex heatmap instead of using DoHeatmap function
# 5. Re-write celldex reference section to allow for additional references
# 6. improve coding in biomart/lowercase human section
# 7. Add scrublet and/or DoubletFinder support for doublet detection
# 8. Change to scientific notation for number on x-axis that tend to get scrunched
# 9. Assess number of samples and dynamically determine figure widths and plot layout for sample stats section.
# 10. Add support for metadata use and scater function,ef:
...
sce.list <- lapply(seurat.list, function(s) Seurat::as.SingleCellExperiment(s))
sce.list <- lapply(sce.list, function(sce) scds::cxds_bcds_hybrid(sce))
...
Create metadata variable and plot filtered vs non-filtered cells.  If doublet detection was performed the cells are already downsamples to 80k and filtered to exclude percent mitochondrial genes above 20% and number of genes below 200.
```{r, mark-filtered}
# nFeature_RNA
...
```{r subset-seurat, eval=params$subset}
seurat <- subset(seurat, downsample=params$downsample)
```
